...
import org.fujaba.graphengine.isomorphismtools.csp.GraphTree;
import org.fujaba.graphengine.isomorphismtools.csp.GraphTreeNode;

public class IsomorphismHandlerCSP extends IsomorphismHandler {

	IsomorphismHandler normalizationFallback = new IsomorphismHandlerSorting();
...
nodeMatch:	for (int j = 0; j < baseGraph.getNodes().size(); ++j) {
		...
		if (!GraphEngine.isConnected(subGraph)) {
			try {
...
//				return null; // TODO: make this compatible for non-connected graphs
				return GraphEngine.getSplitGraphFallback().mappingFrom(subGraph, baseGraph);
			}
		}
		/*
